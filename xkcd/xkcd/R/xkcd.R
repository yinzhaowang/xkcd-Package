...
dxkcd=function(x, sd = 1, log = FALSE, swap.end.points = FALSE){
    ...
    if (sum(i)>0){
        if (log == FALSE & swap.end.points == FALSE){
            y[i]=2*sqrt((-1/2*log2psd2[i]-log(x[i]))*2*sd^2)
        }
...
pxkcd=function(q, sd = 1, log.p = FALSE, swap.end.points = FALSE){
    ...
    if (sum(in0)>0){
        ...
        if (log.p == FALSE & swap.end.points == FALSE){
            temp1=dxkcd(q[in0],sd=sd)/2 
            temp2=pnorm(-temp1 ,mean=0,sd=sd) 
        ...
        #case2
            ...
            if(sum(i3)>0){
                temp11=dxkcd(q[i3],sd=sd,swap.end.points = TRUE)/2 
        ...
        #case3
            ...
            if(sum(i4)>0){
                temp3=log(sqrt(2* pi)*sd *q[i4] )
                p[i4]=log(2/sqrt(pi))+ temp3 + log(1/2-temp3)-1/2*log(-temp3)
            }
            if(sum(i5)>0){
                temp111=dxkcd(q[i5],sd=sd)/2
                temp222=pnorm(-temp111 ,mean=0,sd=sd)
        ...
        #case4
            ...
            if(sum(i2)>0){
                p[i2]=log(sqrt(2*sd^3 *sqrt(2*pi) )*4/3)+3/2*log(q[i2])  
            }
            if(sum(i3)>0){
                temp11=dxkcd(q[i3],sd=sd,swap.end.points = TRUE)/2 
                temp22=pnorm(-temp11 ,mean=0,sd=sd)
